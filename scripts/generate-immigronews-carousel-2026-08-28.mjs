...
const outDir = path.resolve('public/social/2026-08-28-public-charge');
...
const slides = [
  {kicker:'IMMIGRONEWS ALERT', title:'Public charge guidance changes Sept. 18', body:['USCIS says updated guidance applies to many green-card adjustment filings submitted on or after Sept. 18, 2026.'], foot:'Source: USCIS alert, Aug. 18, 2026'},
  {kicker:'WHAT CHANGED', title:'USCIS will use updated factors', body:['Officers review the total record, including age, health, family status, finances, education, skills, and other relevant evidence.'], foot:'Source: USCIS Policy Manual guidance'},
  {kicker:'WHO TO WATCH', title:'Many I-485 applicants are affected', body:['USCIS says adjustment applicants are subject to public charge unless their category is exempt. Refugees, asylees, TPS applicants, SIJ, T, U, and VAWA applicants are listed as exempt.'], foot:'Check your own category carefully'},
  {kicker:'GENERAL INFORMATION', title:'Save the official source', body:['This is general information, not legal advice. For your own case, read the USCIS source and talk to a qualified immigration attorney.'], foot:'Visit immigronews.com • Follow @immigronews'}
];

function wrap(text, maxChars) {
  const words = text.split(/\s+/); const lines=[]; let cur='';
  for (const w of words) { const t = (cur+' '+w).trim(); if (t.length <= maxChars) cur=t; else { if(cur) lines.push(cur); cur=w; } }
...
for (let i=0; i<slides.length; i++) {
  ...
  let titleSvg='', ty=300;
  for (const line of titleLines) { titleSvg += `<text x="110" y="${ty}" font-family="Georgia, 'Times New Roman', serif" font-size="72" font-weight="900" fill="#10233f">${esc(line)}</text>`; ty += 84; }
  const bodySvg = textBlock(s.body, 110, Math.max(560, ty+70), 42, 500, '#172033', i===2 ? 36 : 31, 1.28);
  ...
  await base.composite(composites).toFile(path.join(outDir, `slide-${i+1}.jpg`));
}
const composites = [];
